...
KSQLDB_ENDPOINT = os.getenv('KSQLDB_ENDPOINT_EXTERNAL')
...
def validate_config():
    ...
    required_vars = ['KSQLDB_ENDPOINT_EXTERNAL', 'KSQLDB_API_KEY', 'KSQLDB_API_SECRET']
...
def query_ksqldb(query: str) -> list:
    ...
    endpoint = KSQLDB_ENDPOINT.rstrip('/') if KSQLDB_ENDPOINT else ''
...
def get_sku_availability(sku: str = "", branch: str = "") -> str:
    ...
    """
    try:
        # Always fetch full table, then filter in Python with case-insensitive partial matching
        # This handles branch names like "Dot" matching "DOT Shopping"
        results = query_ksqldb("SELECT * FROM INVENTORY_AVAILABILITY;")

        if sku:
            results = [r for r in results if sku.lower() in r['sku'].lower()]
        if branch:
            results = [r for r in results if branch.lower() in r['branch'].lower()]
